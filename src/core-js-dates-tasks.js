...
function getNextFriday(date) {
  date.setDate(date.getDate() + ((12 - date.getDay() - 1) % 7) + 1);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round(timePassed / (24 * 60 * 60 * 1000)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    checkedDate.getTime() <= end.getTime() &&
    checkedDate.getTime() >= start.getTime()
  );
}
...
function formatDate(date) {
  ...
  return `${newDate.getMonth() + 1}/${newDate.getUTCDate()}/${newDate.getFullYear()}, ${time}`;
}
